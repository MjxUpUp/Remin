...
> **当前状态（2026-09-22）**：v0.2.0 重建之上，v0.3.x 生命周期与 v0.4.0 深度提取/引导体验已发版；主线推进闲时 tick、多格式适配器与 live parity（feat/idle-tick、feat/transcript-adapters、feat/agent-parity）。端到端演练（`scripts/e2e-drill.sh`）17 步全通；全量测试 + `-race` + `make constitution` 全绿；live parity 真机证据：claude 通道通过。
...
| `remin eval run [--suite trust\|roundtrip\|parity\|conflict\|budget\|parity-live\|all]` | 评测（规则可判定、模型无关），JSON 报告；parity 实跑 MCP stdio 通道；`parity-live`（opt-in，不入 all）接入**真实第二 agent 会话**（`claude -p` / `codex exec` 经各自 MCP 通道调 memory_search 逐字回显命中 ID，与核心确定性检索真值逐集比对；通道构造零全局配置污染）；有失败项时退出码 1 |
