...
> **当前状态（2026-09-22）**：v0.2.0 重建 + v0.3.x 生命周期 + v0.4.0 深度提取/引导体验之上，v0.5.0 落地路线图全部六项（闲时 tick、多格式适配器、live parity、uplift 评测、笔记桥、审收 Web 界面）+ 弃权词级判据修复。端到端演练（`scripts/e2e-drill.sh`）20 步全通；全量测试 + `-race` + `make constitution` 全绿；live parity 真机证据：claude 通道通过。
...
| `remin eval run [--suite trust\|roundtrip\|parity\|conflict\|budget\|uplift\|parity-live\|all]` | 评测（规则可判定、模型无关），JSON 报告；parity 实跑 MCP stdio 通道；uplift 端到端任务提升（空库基线 vs 带记忆差值归因/弃权不计分/superseded 零命中/注入面同验）；`parity-live`（opt-in，不入 all）接入**真实第二 agent 会话**（`claude -p` / `codex exec` 经各自 MCP 通道调 memory_search 逐字回显命中 ID，与核心确定性检索真值逐集比对；通道构造零全局配置污染）；有失败项时退出码 1 |
| `remin eval uplift --tasks <file> [--record]` | 任务提升真源实测：自备任务集（JSONL：query/facet/expect_id\|expect_contains\|expect_abstain）跑真源检索面（不写记忆真源；测量非门禁，退出码恒 0）；--record 逐次落 `eval/history.jsonl`（纵向衰减曲线随真实使用累积；wrong-abstain/false-hit 信号随行记录） |
...
| `remin ui [--port N] [--no-open]` | 本地审收 Web 界面（仅绑 127.0.0.1；v2：三栏工作台（批次\|候选流\|上下文栏）/状态带（真源·版本·记忆数·深挖待办徽章 hover 明细）/键盘审收流（`j/k` 移动·`a` 采纳·`r` 拒绝·`?` 帮助）/候选类型色条+长文截断+来源原话折叠/批次动作条拒绝需二次确认+类型分诊下拉/回执带 commit 归因/检索 trust 色条+弃权可解释（三种 reason 各配文案）/单条全貌抽屉（supersession 链）/审收趋势迷你图/亮暗双主题（跟随系统）；写操作与 CLI 同选择器语义单一事实源 + Host/Origin/JSON-only 三层写防线；零前端依赖内嵌单页） |
...
```bash
remin mine --deep          # 本机已装 claude/codex 并登录 → 直接可用（零配置）
# 或走自配端点：
