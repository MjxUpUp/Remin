...
> **当前状态（2026-09-22）**：v0.2.0 重建之上，v0.3.x 生命周期与 v0.4.0 深度提取/引导体验已发版；主线推进闲时 tick、多格式适配器、live parity 与 uplift 评测（feat/idle-tick、feat/transcript-adapters、feat/agent-parity、feat/uplift-eval）。端到端演练（`scripts/e2e-drill.sh`）18 步全通；全量测试 + `-race` + `make constitution` 全绿；live parity 真机证据：claude 通道通过。
